...
if (isset($_POST['saveData']) AND $can_read AND $can_write) {
    ...
    if (empty($memberID) OR empty($memberName)) {
        ...
        if (file_exists(MDLBS.'memo/member_custom_fields.inc.php')) {
            include MDLBS.'memo/member_custom_fields.inc.php';
        }
        ...
        if (isset($_POST['updateRecordID'])) {
            ...
            if ($update) {
                ...
                // write log
                utility::writeLogs($dbs, 'staff', $_SESSION['uid'], 'memo', $_SESSION['realname'].' update member data ('.$memberName.') with ID ('.$memberID.')');
            ...
            if ($insert) {
                ...
                // write log
                utility::writeLogs($dbs, 'staff', $_SESSION['uid'], 'memo', $_SESSION['realname'].' add new member ('.$memberName.') with ID ('.$memberID.')');
    ...
    foreach ($_POST['itemID'] as $itemID) {
        ...
        // write log
        utility::writeLogs($dbs, 'staff', $_SESSION['uid'], 'memo', $_SESSION['realname'].' extends memo for member ('.$mtype_d[1].') with ID ('.$memberID.')');
        $num_extended++;
    }
    ...
    foreach ($_POST['itemID'] as $itemID) {
        ...
        if ($loan_d[2] < 1) {
            if (!$sql_op->delete('member', "member_id='$itemID'")) {
                ...
                // write log
                utility::writeLogs($dbs, 'staff', $_SESSION['uid'], 'memo', $_SESSION['realname'].' DELETE member data ('.$loan_d[1].') with ID ('.$loan_d[0].')');
            }
        } else {
...
<div class="menuBoxInner memberIcon">
	...
	<div class="btn-group">
    <a href="<?php echo MWB; ?>memo/index.php" class="btn btn-default"><i class="glyphicon glyphicon-list-alt"></i>&nbsp;<?php echo __('Member List'); ?></a>
    <a href="<?php echo MWB; ?>memo/index.php?expire=true" class="btn btn-default" style="color: #FF0000;"><i class="glyphicon glyphicon-list-alt"></i>&nbsp;<?php echo __('View Expired Member'); ?></a>
    <a href="<?php echo MWB; ?>memo/index.php?action=detail" class="btn btn-default"><i class="glyphicon glyphicon-plus"></i>&nbsp;<?php echo __('Add New Member'); ?></a>
	</div>
    <form name="search" action="<?php echo MWB; ?>memo/index.php" id="search" method="get" style="display: inline;"><?php echo __('Member Search'); ?> :
...
if (isset($_POST['detail']) OR (isset($_GET['action']) AND $_GET['action'] == 'detail')) {
    ...
    if ($form->edit_mode) {
        ...
        if ($is_expired) {
            ...
            $form->addCheckBox('extend', __('Extend memo'), $chbox_array);
    ...
    $form->addSelectList('memberTypeID', __('memo Type').'*', $mtype_options, $rec_d['member_type_id'],'onChange = "generateMemberID(this.value)"');
    ...
    // member is_pending
    $form->addCheckBox('isPending', __('Pending memo'), array( array('1', __('Yes')) ), $rec_d['is_pending']);
    // member photo
    $str_input = '';
    if ($rec_d['member_image']) {
        $str_input = '<div id="imageFilename"><a href="'.SWB.'images/persons/'.$rec_d['member_image'].'" class="openPopUp notAJAX"><strong>'.$rec_d['member_image'].'</strong></a> <a href="'.MWB.'memo/index.php" postdata="removeImage=true&mimg='.$itemID.'&img='.$rec_d['member_image'].'" loadcontainer="imageFilename" class="makeHidden removeImage">'.__('REMOVE IMAGE').'</a></div>';
    }
    ...
    if ($can_read AND $can_write) {
        $datagrid->setSQLColumn('m.member_id',
            ...
            'mt.member_type_name AS \''.__('memo Type').'\'',
    ...
    if ((isset($_GET['keywords']) AND $_GET['keywords']) OR isset($_GET['expire'])) {
        ...
        if (isset($_GET['expire'])) {
            ...
            echo '<div><input type="button" value="'.__('Extend Selected Member(s)').'" onclick="javascript: if (confirm(\''.__('Are you sure to EXTEND memo for selected members?').'\')) { $(\'#mainContent\').simbioAJAX(\''.MWB.'memo/index.php?expire=1\', { method: \'post\', addData: $(\'#memberList\').serialize() + \'&batchExtend=true\' } ); }" class="button" /></div>';
