      ...
      if ($row_count < $record_offset) {
        ...
        continue;
          ...
          if ($field) {
              ...
              $items = trim($field[17]);
              //DZIKHRI -> Add Some Field 
              $book_id = $field[18]?'\''.$field[18].'\'':'NULL';
              $source = $field[19]?'\''.$field[19].'\'':'NULL';
              $condition = $field[20]?'\''.$field[20].'\'':'NULL';
              $supp_mat = $field[21]?'\''.$field[21].'\'':'NULL';
              $coll_type = (integer)utility::getID($dbs, 'mst_coll_type', 'coll_type_id', 'coll_type_name', $field[22], $ct_id_cache);
              $location = utility::getID($dbs, 'mst_location', 'location_id', 'location_name', $field[23], $loc_id_cache);
              $price = $field[24]?'\''.$field[24].'\'':'NULL';
              $original_price = $field[25]?'\''.$field[25].'\'':'NULL';
              ...
              $sql_str = "INSERT IGNORE INTO biblio (title, gmd_id, edition,
                  ...
                  notes, image, sor, input_date, last_update, supp_mat, book_id)
                      VALUES ($title, $gmd_id, $edition,
                      $isbn_issn, $publisher_id, $publish_year,
                      $collation, $series_title, $call_number,
                      $language_id, $publish_place_id, $classification,
                      $notes, $image, $sor, $curr_datetime, $curr_datetime,$supp_mat, $book_id)";
              
              //DZIKHRI -> DEBUGGING QUERY
              //echo '<script>alert("'.$sql_str.'");</script>';
              //echo $sql_str;
              //exit();
              ...
              if (!$dbs->error) {
                  ...
                  // items
                  if (!empty($items)) {
                      $item_sql = 'INSERT IGNORE INTO item (biblio_id, item_code, book_id, source, cond, call_number, coll_type_id, location_id, price, original_price) VALUES ';
                      $item_array = explode('><', $items);
                      foreach ($item_array as $item) {
                          $item = trim(str_replace(array('>', '<'), '', $item));
                          $item_sql .= " ($biblio_id, '$item', $book_id, $source, $condition, $call_number, $coll_type, '$location', $price, $original_price),";
                      }
                      // remove last comma
                      $item_sql = substr_replace($item_sql, '', -1);
//DZIKHRI -> DEBUGGING QUERY
                      //echo '<script>alert("'.$item_sql.'");</script>';
                      //echo $item_sql;
                      //exit();
