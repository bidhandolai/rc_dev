...
if (isset($_POST['detail']) OR (isset($_GET['action']) AND $_GET['action'] == 'detail')) {
  ...
  if ($can_read AND $can_write) {
    ...
    'mst_gmd.gmd_name AS \''.__('GMD').'\'',
    'IF(COUNT(item.item_id)>0, COUNT(item.item_id), \'<strong style="color: #f00;">'.__('None').'</strong>\') AS \''.__('Copies').'\'',
    'biblio.call_number AS \''.__('Call Number').'\'', // Dzikhri Add
  ...
  $searchable_fields = array('title', 'author', 'subject', 'isbn', 'publisher', 'callnumber');
