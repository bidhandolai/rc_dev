      ...
      if ($field_opts[0] == 1) {
        if ($field == 'edition') {
          ...
          if ($_total_avail < 1) {
            ...
            //EDITED BY DZIKHRI 
            if($_total_avail == 1){
                $item_availability_message = str_replace('{numberAvailable}' , $_total_avail, __('{numberAvailable} copy'));
            }elseif($_total_avail == 0){
                $item_availability_message = str_replace('{numberAvailable}' , $_total_avail, __('{numberAvailable} copy'));
            }else{
                $item_availability_message = str_replace('{numberAvailable}' , $_total_avail, __('{numberAvailable} copies'));
            }
            //END OF EDITED BY DZIKHRI
